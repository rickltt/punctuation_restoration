...
def scl_loss(hidden_states, ground_truth, temperature=0.07, pooling=False):
    sup_con_loss = SupConLoss(contrast_mode='one')
...
if __name__ == '__main__':
    ...
    # loss_fun = SupConLoss(contrast_mode='one', temperature=0.07)
    # data = torch.randn(8,512,768)
    # data = data.view(-1,768).unsqueeze(1)
    # label = torch.randint(0,4,(8,512)).float()
    # label.view(-1)
    # loss = loss_fun(data, label)
    loss_fun = SupConLoss(contrast_mode='all', temperature=0.07)
    good_data = torch.rand(6,1,768)
    # good_data = torch.tensor([[[0., 0.]], [[1, 1.]], [[2., 2.]], [[0., 0.]], [[1, 1.]], [[2., 2.]]])
    good_label = torch.tensor([0, -100, 2, -100, -100, 2.])
    good_loss = loss_fun(good_data, good_label)
